 ...
 * 
 * @copyright (c) 2017, Pablo R. Sousa
 */
class Login {
    ...
    public function CheckLogin() {
        if (empty($_SESSION['userlogin']) || $this->Level  < $_SESSION['userlogin']['level_user'] ):
    ...
    private function setLogin() {
        if (!$this->Email || !$this->Password || !Check::Email($this->Email)):
            $this->Error = ['Informe seu E-mail e senha para efetuar o login!', WS_INFO];
            $this->Result = false;
        elseif (!$this->getUser()):
            $this->Error = ['Os dados informados não são compatíveis!', WS_ALERT];
            $this->Result = false;
        elseif ($this->Result['level_user'] > $this->Level ):
    ...
    private function getUser() {
        ...
        $read = new Read;
        $read->ExeRead("user", "WHERE user_email = :e AND user_password = :p", "e={$this->Email}&p={$this->Password}");
    ...
    private function Execute() {
        ...
        $this->Error = ["Olá {$this->Result['user_name']}, seja bem vindo(a). Aguarde redirecionamento!",$_SESSION['userlogin'], WS_ACCEPT];
        $this->Result = true;
    }
