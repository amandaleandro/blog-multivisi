<?php 
  $tuor = $navAdm->getDado()['tutorial'][0];
  $imgCover = 'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSNNh4aGp56ELWHfEvqgodkbt094ohAtT9moaddw3AoelCKOH4g';
  $imgSingle = explode(',', $tuor['tutorial_cover']);
  $category = $navAdm->getDado()['tutorial_category'];
  $files = $navAdm->getDado()['tutorial_files'];
  $checkCat = $navAdm->getDado()['check_category'];
  $checkFil = $navAdm->getDado()['check_files'];
?>

<form action="" method="post" id="form_tutorials" enctype="multipart/form-data">

  <input type="hidden" name="action" value="<?= ($tuor['tutorial_id'] ? 'tutorial_update' : 'tutorial_create') ?>">
  <input type="hidden" name="user_id" value="<?= $_SESSION['userlogin']['user_id'] ?>">
  <input type="hidden" name="tutorial_id" value="<?= ($tuor['tutorial_id'] ? $tuor['tutorial_id'] : '0') ?>">
    ...
    <div class="tab-content" id="v-pills-tabContent">
      <div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
        <label class="w-100 mb-4 position-relative">
          Nome do Post
          <input type="text" name="tutorial_title" class="form-control rounded-0 name_prod" maxlength="75" value="<?= $tuor['tutorial_title']?>">
        ...
        <label class="w-100">
         Descrição do blog
          <textarea name="tutorial_description"  cols="30" rows="10" class="tinymce"><?= $tuor['tutorial_description']?></textarea>
        </label>
      </div>
      <div class="tab-pane fade p-3" id="v-pills-profile" role="tabpanel" aria-labelledby="v-pills-profile-tab">
        <div class="row">
                  ...
                  <div class="custom-control custom-checkbox">
                    <input type="checkbox" name="categories[]" value="<?= $cat['category_id'] ?>" class="custom-control-input" id="cat_<?= $cat['category_id'] ?>" <?php 
                      if($checkCat){ foreach($checkCat as $checkC){
                        echo ($checkC['tutorial_category_category_id'] == $cat['category_id'] ? 'checked' : false);
                      } }
                    ?>>
          ...
          <div class="col">
            <label class="w-100">
            foto do blog
              <input type="file" class="d-none" id="cover" name="tutorial_cover">
              <div class="coveimg shadow-sm">
                <img src="<?= ($tuor['tutorial_id'] ? '/public/img/upload/' . $imgSingle[0] : $imgCover); ?>" class="mw-100">
              </div>
            </label>
          </div>
        </div>
          ...
          <div class="col-4" style="max-height: 277px; overflow: auto;">
            <div class="card border-0 shadow-sm">
              ...
              <ul class="list-group list-group-flush youtubelist">
                <?php if($tuor['tutorial_id']): foreach(explode(',', $tuor['tutorial_video']) as $video): ?>
             ...
             Arquivos
              ...
              <ul class="list-group list-group-flush listfiles">
                <?php if($files): foreach($files as $file): ?>
                  <li class="list-group-item border-light" style="font-size: 13px">
                  <div class="custom-control custom-checkbox">
                    <input type="checkbox" name="files[]" value="<?= $file['archives_id'] ?>" class="custom-control-input" id="file_<?= $file['archives_id'] ?>" <?php 
                      if($checkFil){ foreach($checkFil as $checkF){
                        echo ($checkF['tutorial_archives_archives_id'] == $file['archives_id'] ? 'checked' : false);
                      } }
                    ?>>
                    <label class="custom-control-label" for="file_<?= $file['archives_id'] ?>"><?= $file['archives_name'] ?></label>
                  </div>
                </li>
                <?php endforeach; endif;?>
              </ul>
          ...
          <div class="col">
            <div class="card border-0 shadow-sm">
              ...
              <div class="custom-control custom-radio ml-3">
                <input type="radio" id="customRadio1" name="tutorial_status" value="1" <?= ($tuor['tutorial_id'] && $tuor['tutorial_status'] == 1 ? 'checked' : false) ?> class="custom-control-input">
                <label class="custom-control-label" for="customRadio1">Online</label>
              </div>
              <div class="custom-control custom-radio ml-3">
                <input type="radio" name="tutorial_status" value="0" id="customRadio2"  <?= ($tuor['tutorial_id'] && $tuor['tutorial_status'] == "0" ? 'checked' : false) ?> name="customRadio" class="custom-control-input">
      ...
      <div class="tab-pane fade" id="v-pills-messages" role="tabpanel" aria-labelledby="v-pills-messages-tab">
        <label class="w-100">
          Tutorial URL
          <input type="text" name="tutorial_url" id="prdurl" class="form-control rounded-0" value="<?= $tuor['tutorial_url'] ?>">
          <small><b>Exemple</b>: <i>this-is-a-tutorial</i></small>
        </label>
        <label class="w-100 mt-4 mb-4 position-relative">
          Seo Description
          <div class="row m-0 align-items-center seodescript">
            <input type="text" name="tutorial_seo_description" class="form-control rounded-0 col" maxlength="155" value="<?= $tuor['tutorial_seo_description']?>">
        ...
        <label class="w-100">
          ...
          <div class="row m-0 align-items-center">
            <div class="tag-container col seo_tags" data-seo="<?= ($tuor['tutorial_id'] ? $tuor['tutorial_seo_tags'] : false) ?>">
        ...
        <label class="position-relative mt-5">
          <input type="submit"  value="Save tutorial" class="btn btn-primary rounded-0 pr-5" id="inputSub">
